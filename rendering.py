...
def world_to_perspective_point(persp_pos,persp_target,world_pt):
    eps = 1/(1 << 16)
    forward = normalize(persp_target - persp_pos)
    world_up = np.array([0, 0, 1], float)
    if abs(np.dot(forward, world_up)) > 0.99: world_up = np.array([0, 1, 0], float)
    right = normalize(np.cross(forward, world_up))
    up = np.cross(right, forward)
...
def direction_to_angles(d) -> np.array:
...
def fisheye_equidistant(p,f:float=1.0) -> np.array:
    theta, phi = direction_to_angles(p)
    r = f * theta
    return np.array([r * cos(phi), r * sin(phi)])

def fisheye_equisolid(p,f:float=1.0) -> np.array:
    theta, phi = direction_to_angles(p)
    r = 2.0 * f * sin(theta / 2.0)
    return np.array([r * cos(phi), r * sin(phi)])

def fisheye_stereographic(p,f:float=1.0) -> np.array:
    theta, phi = direction_to_angles(p)
    r = 2.0 * f * tan(theta / 2.0)
    return np.array([r * cos(phi), r * sin(phi)])

def orthographic_onto_disc(p,f:float=1.0) -> np.array:
    theta, phi = direction_to_angles(p)
    r = f * sin(theta)
    return np.array([r * cos(phi), r * sin(phi)])

def equirectangular(p) -> np.array:
    d = normalize(p)
